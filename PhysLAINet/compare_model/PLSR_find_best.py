...
def objective(trial):
    n_components = trial.suggest_int("n_components", 1, min(len(feature_cols), len(X_train) - 1))
...
y_trainval = y_train
final_model = PLSRegression(n_components=study.best_params["n_components"])
...
def plot_true_vs_pred(y_true, y_pred, dataset_name):
    ...
    plt.tight_layout()
    plt.savefig(f"vs/PLSR_{target_col}_{dataset_name}.png", dpi=300)
    plt.show()

plot_true_vs_pred(y_trainval, y_trainval_pred, "Train_Valid")
plot_true_vs_pred(y_test, y_test_pred, "Test")
joblib.dump(final_model, f"checkpoints/plsr_{target_col}_model.pkl")
joblib.dump(scaler, f"checkpoints/plsr_{target_col}_scaler.pkl")
